...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-gallery-white text-ink border-t border-hairline-silver pt-16 pb-12">
      <div className="w-full max-w-[1400px] mx-auto px-6 lg:px-12 space-y-12">
        {/* Top Intro Section with Logo and Blurb */}
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 pb-10 border-b border-hairline-silver">
          ...
          <p className="font-geist text-[14px] text-slate leading-relaxed max-w-[840px]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-8 py-2">
          {linkColumns.map((col, idx) => (
            <div key={idx} className="space-y-3">
              <h4 className="font-mono-geist text-[12px] uppercase text-apple-blue tracking-wider">
                {col.title}
              </h4>
                ...
                {col.links.map((link, lIdx) => (
                  <li key={lIdx}>
                    ...
                    >
                      ...
                      <ArrowUpRight size={12} className="opacity-0 group-hover:opacity-100 transition-opacity text-apple-blue" />
                    </Link>
                  </li>
                ))}
              </ul>
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <a
            ...
            aria-label="Email"
            className="w-11 h-11 rounded-full border border-hairline-silver bg-studio-mist flex items-center justify-center text-slate hover:text-apple-blue hover:border-apple-blue transition-all duration-150 shadow-md"
          >
            <Mail size={18} />
          </a>
        ...
        <div className="text-center font-mono-geist text-[12px] text-slate pt-2">
          © {new Date().getFullYear()} Made with precision by <span className="text-ink">Qadmas Technologies</span> — Operating in Qatar · UAE · India
        </div>
      </div>
    </footer>
  );
